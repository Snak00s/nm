...
int	nmElf32(Elf32_Ehdr *header, void *map_start, char *flagList, char *filename, int multiFile)
{
	...
	Elf32_Shdr	*shstrtab = (map_start + sections[shstrtabIdx].sh_offset);
	...
	//create the tab of symb
	t_symbol32 **symb = symbCreate32(symtab, strtab, sections, nbr_entry, shstrtab, trueSize, header->e_shnum, checkFlag('a', flagList));
	if (!symb)
	{

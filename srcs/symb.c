...
t_symbol32	**symbCreate32(Elf32_Sym *symtab, Elf32_Sym *strtab, Elf32_Shdr *sections,
	unsigned long nbr_entry, Elf32_Shdr *shstrtab, unsigned long trueSize, int shmax, int aFlag)
{
	t_symbol32 **ret = ft_calloc(trueSize, sizeof(t_symbol32 *));
	if (!ret)
		return (NULL);
